...
let spritesheet = 'desmarkdev-spritesheet'
let spritesheetPath = 'assets/images/spritesheet.png'
...
let Animation= {
    ...
    Rubber :{
        Blue : {
            Move: 'rubber-blue-move',
        },

        Orange : {
            Move: 'rubber-orange-move',
        },

        White : {
            Move: 'rubber-white-move',
        },

        Pink : {
            Move: 'rubber-pink-move',
        },

        Red : {
            Move: 'rubber-red-move',
        },
    }
};
...
function preload ()
{
    ...
    this.load.image("pill", "assets/images/desmark-Dev pill/spr_pill_0.png");
    this.load.image("lifecounter", "assets/images/desmark-Dev life counter/curseurLife.png");
    this.load.image("bonus", "assets/images/desmark-Dev Bonus/potdepeinture.png");
    this.load.image("bonus", "assets/images/desmark-Dev Bonus/pinceau.png");
    this.load.image("superG", "assets/images/desmark-Dev Bonus/supergomme.png");
}
...
{    
    ...
    this.anims.create({
            key: Animation.Rubber.Blue.Move,
    ...
    this.anims.create({
            key: Animation.Rubber.Orange.Move,
    ...
    this.anims.create({
            key: Animation.Rubber.White.Move,
            frames: this.anims.generateFrameNumbers(spritesheet, { start: 4, end: 5 }),
            frameRate: 10,
            repeat: -1
        });
    ...
    this.anims.create({
            key: Animation.Rubber.Pink.Move,
    ...
    let rubbersGroup = this.physics.add.group();
    let i=0;
    let skins=[Animation.Rubber.Blue, Animation.Rubber.Red, Animation.Rubber.Orange , Animation.Rubber.Pink];
     map.filterObjects("Objects", function (value, index, array) {        
        if(value.name == "Rubber") {
            let position = new Phaser.Geom.Point(value.x + offset, value.y - offset);
            let rubber = new Rubber(scene, position, skins[i]);
            rubbers.push(rubber);    
            rubbersGroup.add(rubber.sprite);
            i++;
        }
     });
    ...
    this.physics.add.collider(rubbersGroup, layer1);
    ...
    this.physics.add.overlap(player.sprite, rubbersGroup, function(sprite, rubberSprite) {
        if(player.active) {
            player.die();
            for(let rubber of rubbers) {
                rubber.freeze();
            }   
        }
    }, null, this);
    ...
    gameOverText =  this.add.text(400, 300, 'Game Over ',{fontSize:"32px", fill:'#ffffff'});
...
function update()
{
    ...
    for(let rubber of rubbers) {
        rubber.setDirections(getDirection(map, layer1, rubber.sprite));
    }

    player.setTurningPoint(getTurningPoint(map, player.sprite));

    for(let rubber of rubbers) {
        rubber.setTurningPoint(getTurningPoint(map, rubber.sprite));
    }
